...
class MapDiffTest extends FlatSpec with Matchers {
  ...
  "MapDiff.canPatchToValue" must "return true if the patch can be converted to a value" in {
  ...
  "MapDiff.patchToValue" must "always successfully convert a patch generated by valueToPatch back to value" in {
    aDiff.patchToValue(aDiff.valueToPatch(map1)) should equal(map1)
  }
}
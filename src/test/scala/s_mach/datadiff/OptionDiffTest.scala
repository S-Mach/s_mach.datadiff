...
class OptionDiffTest extends FlatSpec with Matchers {
  ...
  implicit object Diff_Test$ extends Diff[Test,TestDiff] {
    ...
    override def patchToValue(patch: Patch): Test = Test(patch.value.getOrElse(throw new PatchIncompleteValueException))
  }
  ...
  "OptionDiff.canPatchToValue" must "return true if the patch can be converted to a value" in {
  ...
  "OptionDiff.patchToValue" must "convert a patch to value when possible" in {
    val aDiff = implicitly[Diff[Option[Test],Option[TestDiff]]]
  ...
  "OptionDiff.patchToValue" must "always successfully convert a patch generated by valueToPatch back to value" in {
    val aDiff = implicitly[Diff[Option[Int],Option[Int]]]

    aDiff.patchToValue(aDiff.valueToPatch(Some(1))) should equal(Some(1))
    aDiff.patchToValue(aDiff.valueToPatch(None)) should equal(None)
  }
}
